...
class alphaess:
    ...
    async def getOneDayStatsData(self, day) -> Optional(list):
        ...
        try:
            ...
            for unit in units:
                if "sys_sn" in unit:
                    serial = unit["sys_sn"]
                    logger.debug(f"Retreiving single day statistics for Alpha ESS unit {serial}")
                    unit['statistics'] = await self.__single_daily_statistics(serial, day)
                    alldata.append({day: unit})
            return alldata, units
    ...
    async def __single_daily_statistics(self, serial, day):
        """Get single daily energy statistics"""
        ...
        logger.debug("Trying to retrieve single daily stats for serial %s, date %s", serial, day)
